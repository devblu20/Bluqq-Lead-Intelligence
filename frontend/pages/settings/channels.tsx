...
export default function ChannelsSettings() {
  ...
  // ── FIX 3: unmount cleanup to prevent state updates on unmounted component ──
  useEffect(() => {
  ...
  useEffect(() => {
    const load = async () => {
      await Promise.all([loadChannels(), loadTwilioConfig()]);
    };
    load();
  }, []);
  ...
  const loadChannels = async () => {
    try {
      ...
      // ── FIX 1: loading state now actually drives the UI ──
  ...
  const loadTwilioConfig = async () => {
    try {
      ...
      // ── FIX 2: only ignore 404 (not configured yet), surface real errors ──
      if (err.response?.status !== 404) {
        if (mountedRef.current) toast.error('Failed to load Twilio config');
      }
    }
  };
  ...
  const connectWhatsApp = async () => {
    ...
    try {
      ...
      // ── FIX 5: form stays open with user data preserved on failure ──
    } finally {
      setSaving(false);
    }
  };
  ...
  // ── FIX 7: safe helpers — no non-null assertion, consistent active check ──
  const isConnected = (platform: string) =>
    channels.some(c => c.platform === platform && Boolean(c.is_active));

  const getChannel = (platform: string): Channel | undefined =>
    channels.find(c => c.platform === platform && Boolean(c.is_active));

  // ── FIX 4: Boolean() instead of strict === true to handle 1 / "true" etc. ──
  const twilioActive = Boolean(twilioConfig?.is_active);

  // ── FIX 1: show loading skeleton while data loads ──
  if (loading) {
    return (
      <AppLayout>
    ...
    <AppLayout>
      ...
      <div className="ch-page">
        ...
        <div className="ch-card">
          ...
          {/* ── FIX 7: safe optional chaining — no ! assertion ── */}
        ...
        <div className="ch-card">
          ...
          {/* ── FIX 7: guard with getChannel check before rendering ── */}
        ...
        <div className="ch-card">
            ...
            <div className="ch-twilio-connected">
              ...
              <div className="ch-twilio-row">
                ...
                <span className="ch-twilio-val">
                  {twilioConfig.created_at
                    ? new Date(twilioConfig.created_at).toLocaleDateString()
                    : '—'}
                </span>
              </div>
            </div>
          ...
          {!showTwilioForm && (
            twilioActive ? (
              <button className="ch-btn-update" onClick={() => setShowTwilioForm(true)}>
                Update credentials
              </button>
            ) : (
              <button
        ...
        <div className="ch-twilio-help">
          ...
          <p className="ch-help-step">
            <span style={{ color: '#FCA86C' }}>Step 1</span> — Go to{' '}
            <strong style={{ color: '#D1D5DB' }}>console.twilio.com</strong> and sign in or create a free account
          </p>
          <p className="ch-help-step">
            <span style={{ color: '#FCA86C' }}>Step 2</span> — On the dashboard you will find your{' '}
            <strong style={{ color: '#D1D5DB' }}>Account SID</strong> and{' '}
            <strong style={{ color: '#D1D5DB' }}>Auth Token</strong> directly
          </p>
          <p className="ch-help-step">
            <span style={{ color: '#FCA86C' }}>Step 3</span> — Go to Phone Numbers → Manage → Buy a Number to get a Twilio phone number
          </p>
          <p className="ch-help-step">
            <span style={{ color: '#FCA86C' }}>Step 4</span> — The number must be in E.164 format — e.g.{' '}
            <strong style={{ color: '#D1D5DB' }}>+14155552671</strong>
          </p>
          <p className="ch-help-step">
            <span style={{ color: '#FCA86C' }}>Step 5</span> — Fill in the form above and click Save &amp; Activate
          </p>
          <p
            className="ch-help-step"
            style={{
              marginTop: '10px',
              padding: '10px 12px',
          ...
          >
            <span style={{ color: '#FCA86C' }}>Note</span> — For Priya AI calls to work, you must also set{' '}
            <strong style={{ color: '#D1D5DB' }}>TWILIO_ACCOUNT_SID</strong>,{' '}
            <strong style={{ color: '#D1D5DB' }}>TWILIO_AUTH_TOKEN</strong>, and{' '}
            <strong style={{ color: '#D1D5DB' }}>TWILIO_PHONE_NUMBER</strong> in your backend{' '}
            <strong style={{ color: '#D1D5DB' }}>.env</strong> file
          </p>
        </div>
      </div>
    </AppLayout>
...
// ── styles extracted to a constant for readability ──
const styles = `
  ...
  /* FIX 1: loading skeleton styles */
  .ch-skeleton      { height: 20px; background: #1F2937; border-radius: 6px; margin-bottom: 12px; animation: ch-pulse 1.4s ease-in-out infinite; }
  .ch-skeleton-card { height: 100px; background: #111827; border: 1px solid #1F2937; border-radius: 14px; margin-bottom: 16px; animation: ch-pulse 1.4s ease-in-out infinite; }
  ...
  .ch-badge-connected { padding: 4px 12px; border-radius: 99px; font-size: 11px; font-weight: 600; background: rgba(34,197,94,0.1); color: #4ADE80; border: 1px solid rgba(34,197,94,0.2); white-space: nowrap; }
  .ch-badge-not       { padding: 4px 12px; border-radius: 99px; font-size: 11px; font-weight: 600; background: rgba(255,255,255,0.05); color: #6B7280; border: 1px solid #1F2937; white-space: nowrap; }
  ...
  /* FIX 6: overflow-safe form inputs */
  ...
  .ch-input { width: 100%; max-width: 100%; padding: 11px 14px; background: #1E293B; border: 1px solid #374151; border-radius: 9px; color: #F9FAFB; font-size: 14px; font-family: 'Inter', sans-serif; outline: none; transition: all 0.2s; box-sizing: border-box; }
  ...
  /* FIX 5: required field indicator */
  ...
  .ch-optional-badge { font-size: 10px; color: #4B5563; background: #1F2937; border: 1px solid #374151; border-radius: 4px; padding: 1px 6px; margin-left: 6px; vertical-align: middle; }
