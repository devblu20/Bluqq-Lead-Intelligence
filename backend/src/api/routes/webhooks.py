...
def find_lead_by_phone(phone: str, org_id: str) -> dict:
    ...
    possible = set()
    possible.add(digits)                    # 917024506852
    possible.add('+' + digits)              # +917024506852

    if digits.startswith('91') and len(digits) == 12:
        local = digits[2:]                  # 7024506852
        possible.add(local)
        possible.add('+91' + local)
    elif len(digits) == 10:
        possible.add('91' + digits)
        possible.add('+91' + digits)

    placeholders = ', '.join(['%s'] * len(possible))
    result = query(
        f"""
        SELECT * FROM leads
        WHERE org_id = %s AND phone IN ({placeholders})
        ORDER BY created_at DESC LIMIT 1
        """,
        (org_id, *possible), fetch="one"
    )
    return dict(result) if result else None
...
async def receive_whatsapp(request: Request):
    ...
    try:
        ...
        if not prior_outbound:
            print(f"[WEBHOOK] No prior outbound — skipping auto reply for {lead_name}")
            return {"status": "ok"}
